...
class PersonProfile(Base, TimestampMixin):
    ...
    contact: Mapped[ProfileContact | None] = relationship(back_populates="profile", cascade="all, delete-orphan")
    resumes: Mapped[list[Resume]] = relationship(back_populates="profile", cascade="all, delete-orphan")
    facts: Mapped[list[Fact]] = relationship(back_populates="profile", cascade="all, delete-orphan")
...
class ProfileContact(Base, TimestampMixin):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    profile_id: Mapped[int] = mapped_column(ForeignKey("person_profiles.id", ondelete="CASCADE"), unique=True)
...
class Resume(Base, TimestampMixin):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    profile_id: Mapped[int] = mapped_column(ForeignKey("person_profiles.id", ondelete="CASCADE"), index=True)
...
class ResumeSection(Base, TimestampMixin):
    ...
    ai_edit_enabled: Mapped[bool] = mapped_column(Boolean, default=False, nullable=False)
...
class ResumeBlock(Base, TimestampMixin):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    section_id: Mapped[int] = mapped_column(ForeignKey("resume_sections.id", ondelete="CASCADE"), index=True)
    ...
    is_visible: Mapped[bool] = mapped_column(Boolean, default=True, nullable=False)
    ai_edit_enabled: Mapped[bool] = mapped_column(Boolean, default=False, nullable=False)
    ai_edit_mode: Mapped[str] = mapped_column(String(80), default="none", nullable=False)
    ...
    fact_links: Mapped[list[ResumeBlockFactLink]] = relationship(back_populates="block", cascade="all, delete-orphan")
...
class ResumeBullet(Base, TimestampMixin):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    block_id: Mapped[int] = mapped_column(ForeignKey("resume_blocks.id", ondelete="CASCADE"), index=True)
    text: Mapped[str] = mapped_column(Text, nullable=False)
    display_order: Mapped[int] = mapped_column(Integer, default=0, nullable=False)
    is_visible: Mapped[bool] = mapped_column(Boolean, default=True, nullable=False)
    ai_edit_enabled: Mapped[bool] = mapped_column(Boolean, default=False, nullable=False)
    fact_link_required: Mapped[bool] = mapped_column(Boolean, default=True, nullable=False)

    block: Mapped[ResumeBlock] = relationship(back_populates="bullets")
    fact_links: Mapped[list[ResumeBulletFactLink]] = relationship(back_populates="bullet", cascade="all, delete-orphan")
...
class Fact(Base, TimestampMixin):
    __tablename__ = "facts"
    __table_args__ = (UniqueConstraint("profile_id", "fact_key", name="uq_profile_fact_key"),)
    ...
    allowed_claim_level: Mapped[str] = mapped_column(String(40), default=ClaimLevel.MENTION_ONLY.value, nullable=False)
...
class ResumeBulletFactLink(Base):
    ...
    bullet_id: Mapped[int] = mapped_column(ForeignKey("resume_bullets.id", ondelete="CASCADE"), primary_key=True)
    fact_id: Mapped[int] = mapped_column(ForeignKey("facts.id", ondelete="CASCADE"), primary_key=True)
...
class ResumeBlockFactLink(Base):
    ...
    block_id: Mapped[int] = mapped_column(ForeignKey("resume_blocks.id", ondelete="CASCADE"), primary_key=True)
    fact_id: Mapped[int] = mapped_column(ForeignKey("facts.id", ondelete="CASCADE"), primary_key=True)
...
class Application(Base, TimestampMixin):
    ...
    profile_id: Mapped[int] = mapped_column(ForeignKey("person_profiles.id", ondelete="CASCADE"), index=True)
    resume_id: Mapped[int] = mapped_column(ForeignKey("resumes.id", ondelete="CASCADE"), index=True)
    application_number: Mapped[int] = mapped_column(Integer, nullable=False, unique=True)
    ...
    status: Mapped[str] = mapped_column(String(60), default=ApplicationStatus.JOB_SAVED.value, nullable=False)

    profile: Mapped[PersonProfile] = relationship()
    resume: Mapped[Resume] = relationship()
    requirements: Mapped[list[ExtractedJobRequirement]] = relationship(back_populates="application", cascade="all, delete-orphan")
...
class TailoringRun(Base):
    ...
    model: Mapped[str] = mapped_column(String(120), default="fake-deterministic", nullable=False)
    warnings_json: Mapped[Any] = mapped_column(JSON, default=list, nullable=False)
    created_at: Mapped[datetime] = mapped_column(DateTime, server_default=func.now(), nullable=False)
    completed_at: Mapped[datetime | None] = mapped_column(DateTime)

    proposals: Mapped[list[AiChangeProposal]] = relationship(back_populates="tailoring_run", cascade="all, delete-orphan")
...
class AiChangeProposal(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    tailoring_run_id: Mapped[int] = mapped_column(ForeignKey("tailoring_runs.id", ondelete="CASCADE"), index=True)
    ...
    requirement_ids_json: Mapped[Any] = mapped_column(JSON, default=list, nullable=False)
    ...
    status: Mapped[str] = mapped_column(String(40), default=ProposalStatus.PROPOSED.value, nullable=False)
    created_at: Mapped[datetime] = mapped_column(DateTime, server_default=func.now(), nullable=False)
...
class CoverLetter(Base, TimestampMixin):
    ...
    profile_id: Mapped[int] = mapped_column(ForeignKey("person_profiles.id", ondelete="CASCADE"), index=True)
    resume_id: Mapped[int] = mapped_column(ForeignKey("resumes.id", ondelete="CASCADE"), index=True)
    prompt_version: Mapped[str] = mapped_column(String(80), default="cover-letter-v1", nullable=False)
