...
TargetType = Literal["resume_bullet", "resume_block", "resume_block_title", "skills_set"]
Operation = Literal["rewrite", "create", "hide", "reorder", "update_title", "update_skills_set"]
...
class AiChangeProposalSchema(BaseModel):
    ...
    def after_text_required_for_content_changes(cls, value: str, info):  # type: ignore[no-untyped-def]
        ...
        if operation in {"rewrite", "create", "update_title", "update_skills_set"} and not value.strip():
