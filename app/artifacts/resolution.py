...
def resolve_artifact_path_under_applications_dir(
    *,
    ...
    if (
        Path(stored_relative_path).is_absolute()
        or PurePosixPath(stored_relative_path).is_absolute()
    ):
    ...
    if not parts or parts[0] != APPLICATIONS_ARTEFACT_ROOT:
        raise UnsafeArtifactPathError(
            "Artefact paths must be relative to the applications artefact root."
        )
    ...
    if candidate_path != resolved_applications_dir and (
        resolved_applications_dir not in candidate_path.parents
    ):
