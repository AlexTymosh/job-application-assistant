...
def test_load_fact_bank_strips_fact_text_fields(tmp_path: Path) -> None:
...
""",
    ...
    fact_bank = load_fact_bank(path)

    assert fact_bank.facts[0].id == "fact_python_001"
    assert fact_bank.facts[0].name == "Python"
    assert fact_bank.facts[0].evidence == "Used in a portfolio project."


def test_load_fact_bank_rejects_empty_facts_list(tmp_path: Path) -> None:
    path = write_fact_bank(tmp_path, "facts: []\n")
